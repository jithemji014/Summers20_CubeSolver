class cubie{
  PMatrix3D matrix;                                                       
  int x=0;                       //Track each x, y, z index values 
  int y=0;
  int z=0;
  color c;
 ...
 void update(int x, int y, int z){                //For updating the matrix
   matrix.reset();
 ...
 noFill();                             //disable the filling geometry
 stroke(0);                           //coloring the outlines
 strokeWeight(0.1);                   
 pushMatrix();                          // Saves the current coordinate system to stack
 applyMatrix(matrix);                  
